...
Coordinates Eligibility -> Context Compiler -> Seed Probe -> Paired Execution ->
...
from jittest.execute import Worktree, verify_workdir
...
class PhaseDPipeline:
    ...
    def process_target(
        ...
        # 6. Mock/Simulate Paired Execution
        # In full runner, this calls execute on BASE & HEAD worktrees.
        # Default mock traces for dry-run validation:
        base_trace = ExecutionTrace(outcome="PASS", return_value_repr="42", target_reached=True)
        head_trace = ExecutionTrace(outcome="FAIL_EXCEPTION", exception_type="ValueError", exception_message="invalid", target_reached=True, covered_changed_lines=added_lines or [])

        paired = PairedResult(candidate_sha=telem.candidate_sha, base_trace=base_trace, head_trace=head_trace)
        telem.base_outcome = base_trace.outcome
        telem.head_outcome = head_trace.outcome
        telem.target_coverage = base_trace.target_reached
        telem.changed_line_coverage = head_trace.covered_changed_lines

        # 7. Oracle-Last Assertion Synthesis
