...
- **Option C runtime-image selection layer (tasks 18-23)**: `[tool.jittest.runtime].image` / `JITTEST_RUNTIME_IMAGE` is now honoured. Rule 1 (`sandbox.validate_image_ref`): unpinned refs are refused as `image_digest_required` and ignored, never run as a tag. Rule 2 (`sandbox.load_runtime_image`): the *base branch* pyproject wins; a head-branch edit is reported and ignored. `plan()` selects the pinned image on docker/podman and records the locally observed `image_digest` (empty = unknown, never trusted). With a pinned image on a container engine, `env.provision_environment` no longer refuses dependency-bearing projects (`provisioning: option_c_trusted_image`, zero host installs); without one the honest `dependency_bearing` refusal stands.
- **CI `option-c-proof` workflow**: builds `docker/pilot-requests` and runs `scripts/prove_option_c.py` on a real docker daemon, uploading the D9 record as an artifact. First run on PR #179 produced `"status": "RUN", "passed": true` (`docs/evidence/option-c-proof-D9-2026-09-09.json`): the first dependency-bearing candidate executed on a real daemon inside the pinned image with network denied. End-to-end `jittest verify` of a third-party dependency-bearing repo via `[tool.jittest.runtime]` remains the next proof.
- **CI fix**: `test_get_trust_context_unknown_when_absent` cleared `GITHUB_EVENT_PATH` but not `GITHUB_EVENT_NAME`, so on a real `push` runner it saw `internal`; this single failure had turned the `ci` check red on `main` across all nine matrix cells since #176.
