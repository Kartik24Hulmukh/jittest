...
- **CI `option-c-proof` workflow**: builds `docker/pilot-requests` and runs `scripts/prove_option_c.py` on a real docker daemon, uploading the D9 record as an artifact. D9 is closed only by a record that says `RUN`, never by this changelog line.
