"""Update phase-d-development-replay.json with provenance and regenerate RESULTS.md strictly from artifact."""
...
# Programmatic provenance via subprocess
...
# Inject provenance into artifact
replay_data["git_commit_sha"] = head_sha
replay_data["git_tree_sha"] = tree_sha
replay_data["preregistration_sha"] = prereg_sha
replay_data["manifest_sha256"] = manifest_sha256

replay_path.write_text(json.dumps(replay_data, indent=2), encoding="utf-8")
print("Updated phase-d-development-replay.json with provenance.")

# Generate RESULTS.md strictly from replay_data
meter_cost = replay_data["meter_cost_usd_delta"]
med_cost = replay_data["median_cost_usd"]
p95_runtime = replay_data["p95_runtime_s"]
total_wall = replay_data["total_wall_clock_s"]
meter_calls = replay_data["meter_calls_delta"]
...
for r in replay_data["rows"]:
    ...
    b_sha = r["base_sha"]
    h_sha = r["head_sha"]
    c_sha = r.get("candidate_sha", "")[:16] or "N/A"
    calls = sum(r["model_calls_by_stage"].values())
...
| **Provider Request Count (Delta)** | **{meter_calls} calls** | Metered | **VERIFIED** |
| **Provider Spend (Delta)** | **${meter_cost:.7f} USD** | Metered | **VERIFIED** |
| **Median Cost per Row** | **${med_cost:.7f} USD** | `< $0.25` | **VERIFIED** |
| **p95 Wall-Clock Runtime** | **{p95_runtime:.2f} seconds** | `< 600s` | **VERIFIED** |
| **Total Wall-Clock Runtime** | **{total_wall:.2f} seconds** | Metered | **VERIFIED** |
...
print("Regenerated RESULTS.md strictly from phase-d-development-replay.json.")
