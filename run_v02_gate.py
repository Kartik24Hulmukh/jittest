...
and publishes signed evidence artifacts to docs/evidence/v0.2/.
"""
...
DOCS_EVIDENCE_DIR = Path(r"C:\Users\praja\src\jittest\docs\evidence\v0.2")
...
PROFILES = [
    {
        ...
        "test_file": FLASK_REPO / "tests" / "test_cli.py",
    },
    {
        "id": "flask_02",
        ...
        "test_file": FLASK_REPO / "tests" / "test_basic.py",
    },
    {
        "id": "flask_03",
    ...
    },
    {
        ...
        "test_file": FLASK_REPO / "tests" / "test_blueprints.py",
    },
    {
        "id": "flask_05",
        ...
        "test_file": FLASK_REPO / "tests" / "test_views.py",
    },
]
...
def main():
    ...
    for r in results:
        print(f"[{'PASS' if r['receipt_valid'] else 'FAIL'}] {r['name']}: {r['verdict']} ({r['disposition']}) -> {r['artifact_path']}")

    all_valid = all(r["receipt_valid"] for r in results)
    print(f"\nHard Gate Status: {'PASSED 5/5 PROFILES' if all_valid else 'FAILED'}")
