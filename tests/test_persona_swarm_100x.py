...
def test_100_personas_at_100x_zero_panics_and_sub_200ms_recovery():
...
def test_swarm_report_is_deterministic_for_a_fixed_seed():
    swarm_mod = _load_swarm()
...
def test_frozen_launch_artefact_passed():
...
def test_run_bounded_has_no_polling_sleep():
...
def test_run_bounded_timeout_recovers_within_200ms():
...
def test_run_bounded_malformed_script_is_a_result_not_a_panic():
...
def test_probe_app_never_raises_and_always_answers_json(path, method, ready, drain):
    app = ProbeApp()
    app.set_ready(ready)
    if drain:
        app.drain()
