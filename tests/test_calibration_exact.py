"""Unit test enforcing exact byte-for-byte 40-character match of all 20 W1-R6 calibration pairs.

NOTE: W1-R6 is an internal calibration dataset used strictly for risk-gate tuning
and calibration evidence. It is NOT public catch-rate or benchmark evidence.
"""
...
class ExactCalibrationGuardTests(unittest.TestCase):
    def test_calibration_artifact_and_sidecar_exact_match(self):
        artifact_path = Path("eval/artifacts/flask-fp-ladder-w1r6-sanitized.json")
        sidecar_path = Path("eval/artifacts/flask-fp-ladder-w1r6-sanitized.json.sha256")

        self.assertTrue(artifact_path.exists(), "Missing artifact file!")
        self.assertTrue(sidecar_path.exists(), "Missing artifact sidecar file!")
        ...
        self.assertEqual(len(ancestry), 20, "Must have exactly 20 ancestry pairs!")

        seen_pairs = set()
        for i, (expected_base, expected_head) in enumerate(EXPECTED_W1R6_PAIRS):
            row = ancestry[i]
            base_sha = row.get("base_sha", "")
            head_sha = row.get("head_sha", "")

            self.assertTrue(
                HEX_40_RE.match(base_sha),
                f"Row {i} base_sha '{base_sha}' is not a 40-character lowercase hex string!",
            )
            self.assertTrue(
                HEX_40_RE.match(head_sha),
                f"Row {i} head_sha '{head_sha}' is not a 40-character lowercase hex string!",
            )

            self.assertEqual(base_sha, expected_base, f"Row {i} base_sha mismatch!")
            self.assertEqual(head_sha, expected_head, f"Row {i} head_sha mismatch!")

            pair = (base_sha, head_sha)
            self.assertNotIn(pair, seen_pairs, f"Duplicate pair detected at index {i}: {pair}")
            seen_pairs.add(pair)
